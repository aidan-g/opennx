...
class RmRfTraverser : public wxDirTraverser
{
    public:
        ...
        ~RmRfTraverser()
        {
            ...
            while (n >= 0) {
                ::myLogTrace(MYTRACETAG, wxT("Removing file %s"), m_aFiles[n].wx_str());
                ::wxRemoveFile(m_aFiles[n--]);
            }
            n = m_aDirs.GetCount() - 1;
            while (n >= 0) {
                ::myLogTrace(MYTRACETAG, wxT("Removing dir %s"), m_aDirs[n].wx_str());
                ::wxRmdir(m_aDirs[n--]);
            }
        }
        ...
        {
            ::myLogTrace(MYTRACETAG, wxT("going to delete file %s"), filename.wx_str());
        ...
        virtual wxDirTraverseResult OnDir(const wxString& dirpath)
        {
            ::myLogTrace(MYTRACETAG, wxT("going to delete dir %s"), dirpath.wx_str());
            m_aDirs.Add(dirpath);
            return wxDIR_CONTINUE;
        }
        ...
        if (m_re->Matches(tmp)) {
            ...
            if (it == m_sessions->end()) {
                ...
                ::myLogTrace(MYTRACETAG,
                        ...
                        m_re->GetMatch(tmp,2).wx_str(), m_re->GetMatch(tmp,3).wx_str(),
                        m_re->GetMatch(tmp,4).wx_str(), port, md5.wx_str());
    ...
    while (!finished) {
        ...
        for (it = m_sessions->begin(); it != m_sessions->end(); ++it) {
            if (it->second.bGetTouched()) {
                ...
                ::myLogTrace(MYTRACETAG, wxT("Session '%s' disappeared"), md5.wx_str());
                finished = false;
                if (m_pAdminHandler) {
                    wxCommandEvent ev(wxEVT_SESSIONLIST_ACTION, wxID_ANY);
...
SessionList::CleanupDir(wxString &dir)
{
    if (!dir.IsEmpty()) {
        {
            ::myLogTrace(MYTRACETAG, wxT("CleanupDir '%s'"), dir.wx_str());
            wxDir d(dir);
            RmRfTraverser t;
            d.Traverse(t);
        }
